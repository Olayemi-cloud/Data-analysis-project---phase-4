...
#Take one completed imputed dataset for plotting (common practice for figures)
...
#Reshape to long format and recode time points
...
#Position labels below boxplots
...
#Pool results for overall model summary
...
#Convert imputed mids object to long format and add BMI group
...
#Calculate average sample size per treatment group × BMI group across imputations
n_table <- long_data %>%
...
#Merge sample size `n` into the estimated marginal means table
bmi_emm_df <- bmi_emm_df %>%
  left_join(n_table, by = c("treatment_group", "bmi_group"))

# View final output table with n included
print(bmi_emm_df)
...
#Convert mids object to long format and add age groups
long_data <- complete(imp, "long", include = TRUE)
